...
char * font_file   = "../resources/font/hzk16";
char * output_dir  = "../language";
char * output_file = "chinese";
char * input_file  = "infor.txt";
char * charset_name = "gb2312";
char * show_string = "enable";
char   show_switch = 1;
...
static int create_fnt(unsigned char * buffer,char * dir,char * name,char * infile)
{
    ...
    while(buffer[i])
    {
        if(buffer[i]>=0x80)
        {
            ...
            long pos = ((buffer[i]-161) * 94 + (buffer[i+1]-161)) * 32;
...
{
    char*  param[] = {"-charset", "-show", "-o", "-m", "-p", "-font"};
    int i,count = sizeof(param)/sizeof(param[0]);
    char* * dest[] = {
        &charset_name,    &show_string,
        &output_file,     &input_file,
        &output_dir,      &font_file
    };
    ...
    if (0 == strcmp(show_string, "enable"))
    {
        show_switch = 1;
    }
    else if (0 == strcmp(show_string, "disable"))
    {
        show_switch = 0;
    }
    else
    {
        printf(MSG_INVALID_VALUE,"-show", show_string);
        return INVALID_PARAM;
    }
