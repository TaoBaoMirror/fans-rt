...
#define MSG_INVALID_VALUE                                                   \
    " 错误：参数 %s 选择的 %s 是一个无效值\n"                               \
    "       如有疑问，请用 --help 获得帮助。\n"
#define MSG_FAILED_OPEN                                                     \
    " 错误：无法打开文件 <%s> 请检查文件是否存在。\n"                       \
    "       如有疑问，请用 --help 获得帮助。\n"
#define MSG_HELP                                                            \
    " 创建 Boot sector 引导代码的简易字体库信息\n\n"                        \
    " charfont [param] <value>\n\n"                                         \
    " 可选参数如下：\n"                                                     \
    " --help               显示帮助信息。\n"                                \
    " -charset <name>      选择字符集，默认为 gb2312(中文)。\n\n"           \
    " -show <switch>       选择是否输出文件信息(默认 enable)，可选:\n"    \
    "                           -show enable    允许输出\n"                 \
    "                           -show disable   禁止输出\n\n"               \
    ...
    "                      的 16 点阵字体文件，繁体、简体均可。\n\n"        \
    "  例如： charset -m error.txt -p .. -o simpchin -font hzk16f\n"
