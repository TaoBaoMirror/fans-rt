...
$(CONFIGS_PATH):
	$(COMMAND_SHOW)$(ECHO) "Create directory [$@] ..."
	$(COMMAND_SHOW)$(MD) -p $@
...
$(DEPENDS_PATH)/%.d: %.c $(CONFIGS_MAKE)
	...
	$(COMMAND_SHOW)$(CC) $(CC_FLAGS) $(CC_INCLUDES) -MM $< >> $@

config: $(CONFIGS_PATH) $(OBJECTS_PATH) $(TARGETS_PATH) $(DEPENDS_PATH) $(TOOLSET_ROOT) $(TOOLSET_PATH)\
	...
	$(COMMAND_SHOW)$(ECHO) "" >> $(CURRENT_MAKE)
	$(COMMAND_SHOW)$(ECHO) ".PHONY: $(SUBDIRS) $(TOOLSET_MAKE) clean" >> $(CURRENT_MAKE)
...
remove_config:
	...
	$(COMMAND_SHOW)$(ECHO) "if [ -d \"$(OBJECTS_PATH)\" ]; then" > ./temp.sh
	$(COMMAND_SHOW)$(ECHO) "	$(ECHO) Remove directory [$(OBJECTS_PATH)]" >> ./temp.sh
	$(COMMAND_SHOW)$(ECHO) "	$(RD) --ignore-fail-on-non-empty $(OBJECTS_PATH)" >> ./temp.sh
ifneq ($(TARGETS_PATH),)
	$(COMMAND_SHOW)$(ECHO) "	$(ECHO) Remove directory [$(TARGETS_PATH)]" >> ./temp.sh
	$(COMMAND_SHOW)$(ECHO) "	$(RD) --ignore-fail-on-non-empty $(TARGETS_PATH)" >> ./temp.sh
endif
ifneq ($(DEPENDS_PATH),)
	$(COMMAND_SHOW)$(ECHO) "	$(ECHO) Remove directory [$(DEPENDS_PATH)]" >> ./temp.sh
	$(COMMAND_SHOW)$(ECHO) "	$(RD) --ignore-fail-on-non-empty $(DEPENDS_PATH)" >> ./temp.sh
endif
ifneq ($(TOOLSET_PATH),)
	$(COMMAND_SHOW)$(ECHO) "	$(ECHO) Remove directory [$(TOOLSET_PATH)]" >> ./temp.sh
	$(COMMAND_SHOW)$(ECHO) "	$(RD) --ignore-fail-on-non-empty $(TOOLSET_PATH)" >> ./temp.sh
endif
	$(COMMAND_SHOW)$(ECHO) "fi" >> ./temp.sh
	$(COMMAND_SHOW)./temp.sh
	$(COMMAND_SHOW)$(RM) -rf ./temp.sh
